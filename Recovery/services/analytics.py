...
def prepare_recovery_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cleaned.loc[(cleaned["Status"] == "Paid") | (left_to_pay <= 0), "recovery_segment"] = "Paid / Completed"
    ...
    cleaned.loc[(amount_paid == 0) & (~cleaned["problem_case"]), "recovery_segment"] = "Defaulter"

    cleaned["risk_level"] = "Medium Risk"
    cleaned.loc[(cleaned["recovery_segment"] == "Paid / Completed"), "risk_level"] = "Low Risk"
    cleaned.loc[(cleaned["recovery_segment"].isin(["Defaulter", "Problem Case"])), "risk_level"] = "High Risk"
